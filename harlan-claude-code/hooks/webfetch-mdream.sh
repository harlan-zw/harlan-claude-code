...
if [[ "$url" =~ ^https?://github\.com/([^/]+)/([^/]+)/blob/(.+)$ ]]; then
  ...
  warn "GitHub blob URL returns HTML. Use raw URL: curl -sL '$raw_url'"
fi

# GitHub tree URLs (directories) → no raw equivalent
if [[ "$url" =~ ^https?://github\.com/([^/]+)/([^/]+)/tree/ ]]; then
  warn "GitHub tree URL is a directory listing (HTML). Use GitHub API or clone the repo instead."
fi
...
if [[ -n "$url" ]]; then
  warn "For HTML pages consider: mcurl $url"
fi
