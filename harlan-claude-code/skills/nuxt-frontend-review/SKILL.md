...
!`GH=$(jq -r '.git_hash // empty' .claude/context/build-handoff.json 2>/dev/null); FILES=$(if [ -n "$GH" ]; then git diff --name-only "$GH" -- '*.vue' '*.ts' '*.css' 2>/dev/null; else git diff --name-only HEAD -- '*.vue' '*.ts' '*.css' 2>/dev/null; fi); echo "$FILES" | head -30; TOTAL=$(echo "$FILES" | grep -c . 2>/dev/null || echo 0); [ "$TOTAL" -gt 30 ] && echo "... and $((TOTAL - 30)) more files (truncated)"`
