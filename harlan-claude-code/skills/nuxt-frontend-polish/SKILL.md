---
description: Polish and refine existing Nuxt UI interfaces. Use when user says "improve", "polish", "refine", "enhance", "make it feel better", "add motion", "fix the UX", "make it less boring", "looks generic", "feels flat", "needs life", "too plain", or when micro UX improvements are needed on existing pages or components.
user_invocable: true
...
- **Over-polishing kills distinctiveness** -- adding motion, gradients, and effects to everything makes it generic. Pick 1-2 areas to elevate, leave the rest alone.
- **Motion-v SSR hydration** -- animations that run on mount can cause hydration mismatches. Use `ClientOnly` wrapper or `onMounted` for entrance animations.
- **OKLCH browser support** -- works in modern browsers but breaks in older Safari. Always provide fallback via `@supports` or stick to hex/hsl for critical colors.
- **Font loading flash** -- adding custom fonts without `font-display: swap` causes invisible text. Nuxt Fonts handles this, but only if fonts are registered there (not via CSS `@import`).
- **Container queries need `container-type`** -- forgetting to set `container-type: inline-size` on the parent is the #1 reason container queries silently don't work.
- **Deleting "ugly" code that works** -- resist the urge to refactor components while polishing. Polish is visual refinement, not code cleanup.
